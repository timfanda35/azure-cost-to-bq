...
def test_run_returns_200_on_success(client):
    result = {"run_id": "20260623-1", "periods_loaded": 6, "periods_skipped": 0}
    with patch("main.run_pipeline", return_value=result) as mock_pipeline:
        resp = client.post("/run")
    assert resp.status_code == 200
    assert resp.json() == result
    mock_pipeline.assert_called_once_with(report=None, partition=None)


def test_run_passes_report_and_partition(client):
    with patch("main.run_pipeline", return_value={"run_id": "x"}) as mock_pipeline:
        resp = client.post("/run", json={"report": "focus", "partition": "2026-05"})
    assert resp.status_code == 200
    mock_pipeline.assert_called_once_with(report="focus", partition="2026-05")
